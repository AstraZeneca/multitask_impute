...
def create_dataset(param):
    ...
    """
    dataset_class = find_dataset_using_name(param.omics_mode) #param.omics_mode is one of a, b, c, ab, abc
...
def create_separate_dataloader(param):
    ...
    if param.not_stratified:
        ...
        print('type of full_idx', type(full_idx))
        print('shape of full_idx', full_idx.shape)
        print(targets[0:20])
        print(full_idx[0:20])
        train_idx, test_idx = train_test_split(full_idx,
                                               test_size=param.test_ratio,
...
def create_separate_dataloader_cv(param):   # my addition to allow us to do cross validation to get variability
    """
    Create set of dataloader (train, val, test).
    """
    full_dataset = create_dataset(param)
    ...
    if param.not_stratified:
        ...
        kf.get_n_splits(full_idx)
        print(kf)
        for fold, (tr_idx, te_idx) in enumerate(kf.split(full_idx), 1):
            if fold == param.cv_fold_index: #PUT IN NEW PARAMETER saying which repeat of the 3 you are currently doing
                train_idx, test_idx = tr_idx, te_idx
                print('cv train shape', train_idx.shape)
                print('cv test shape', test_idx.shape)
        #train_idx, test_idx = train_test_split(full_idx,
        #                                       test_size=param.test_ratio,
        #                                       train_size=param.train_ratio,
        #                                       shuffle=True, random_state=0)
        ...
        if param.downstream_task == 'classification':
            ...
            targets = full_dataset.labels_array[0]
        print('type of targets', type(targets))
        print('shape of full targets', targets.shape)
        print('type of full_idx', type(full_idx))
        print('shape of full_idx', full_idx.shape)
        print(targets[0:20])
        print(full_idx[0:20])
        ...
        skf.get_n_splits(full_idx, targets)
        print(skf)
        for fold, (tr_idx, te_idx) in enumerate(skf.split(full_idx, targets), 1):
            if fold == param.cv_fold_index: # which repeat of the 5 you are currently doing
                train_idx, test_idx = tr_idx, te_idx
                print('cv train shape', train_idx.shape)
                print('cv test shape', test_idx.shape)
        #train_idx, test_idx = train_test_split(full_idx,
